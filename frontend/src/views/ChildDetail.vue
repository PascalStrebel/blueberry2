    ...
    <ion-accordion-group>
      <ion-accordion v-for="category in categories" :key="category">
        ...
        <ion-item v-for="observation in observations.filter(obs => obs.category === category)" class="ion-padding"
          ...
          <ion-button @click="completeObservation(child, observation)" :disabled="observationCompleted(observation)">Complete</ion-button>
        </ion-item>
...
function observationCompleted(observation: Observation): boolean {
  return childObservations.value.filter(co => co?.observation.id === observation.id && co.child.id == child.value.id).length > 0;
}
</script>
